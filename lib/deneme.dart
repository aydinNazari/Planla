...
class Deneme {
  ...
  List<TodayModel> profileScreenGetTodayList(BuildContext context){
    ProviderUser providerUser=Provider.of<ProviderUser>(context,listen: false);
    List<TodayModel> listToday=[];
    for (int i = 0; i < providerUser.getTankList.length; i++) {
      if (providerUser.getTankList[i].done) {
        listToday.add(providerUser.getTankList[i]);
      }
    }
  ...
  Future<void> getTankList(
    ...
    try {
      var querySnapshot =
      await firestore.collection('tank').doc(providerUser.user.uid).get();
      ...
      if (querySnapshot.exists) {
        ...
        nestedMaps.forEach((key, value) {
          ...
          TodayModel todayModel = TodayModel(
            text: value?['text'] ?? '',
            dateTime:
            value?['dateTime'] ?? Timestamp.fromMillisecondsSinceEpoch(0),
